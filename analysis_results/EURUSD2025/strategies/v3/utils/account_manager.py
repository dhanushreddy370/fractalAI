...
class TradingAccount:
    ...
    def open_trade(self, entry_time, entry_price, sl_price, trade_type, reason, context={}, tp_price=None):
        ...
        slippage = np.random.uniform(0.00002, 0.00005)  # 0.2-0.5 pips slippage
        
        if trade_type.upper() == 'BUY':
            effective_entry_price = entry_price + spread + slippage
        else:  # SELL
            effective_entry_price = entry_price - spread - slippage
        ...
        stop_loss_pips_val = abs(effective_entry_price - sl_price) / PIP_SIZE
        ...
        if tp_price is None:
            ...
            tp_price = effective_entry_price + tp_distance if trade_type.upper() == 'BUY' else effective_entry_price - tp_distance

        # Calculate the partial take profit level (at 1:1 Risk/Reward)
        partial_tp_distance = stop_loss_pips_val * PIP_SIZE
        partial_tp_price = effective_entry_price + partial_tp_distance if trade_type.upper() == 'BUY' else effective_entry_price - partial_tp_distance
